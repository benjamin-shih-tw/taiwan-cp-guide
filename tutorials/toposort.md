# 拓撲排序 (Topological Sort) (單元教材)

本單元 **「拓撲排序 (Topological Sort)」** 專屬客製化教材目前正在全力撰寫與校對中！
...
1. **掌握單元核心概念**：將有向無環圖 (DAG) 的頂點依照依賴關係線性排序。Kahn 演算法（BFS 入度）與 DFS 法，是有向圖 DP（Shortest Path on DAG）的基礎。
