# 斜率優化 (Convex Hull Trick / Li Chao Tree) (單元教材)

本單元 **「斜率優化 (Convex Hull Trick / Li Chao Tree)」** 專屬客製化教材目前正在全力撰寫與校對中！
...
1. **掌握單元核心概念**：將 DP 轉移中形如 $f(i) = \min_j(b(j) \cdot x(i) + c(j))$ 的式子視為直線的最小值問題，用凸包維護使每次查詢 $O(\log N)$ 甚至 $O(1)$。
