#!/usr/bin/env python3
"""Save notion-fetch JSON files from mcp_ingest/ or notion_raw/ into notion-cache/."""
import json
import sys
...
def load_raw(pid: str) -> dict | None:
    for base in (ROOT / "mcp_ingest", ROOT / "notion_raw", Path("/tmp/notion_mcp")):
        f = base / f"{pid}.json"
        if f.exists():
...
def main() -> None:
    if len(sys.argv) > 1 and sys.argv[1] == "--stdin-jsonl":
        for line in sys.stdin:
            line = line.strip()
            if not line:
                continue
            data = json.loads(line)
            pid = data.pop("id", None) or data.get("page_id")
            if not pid:
    ...
    print(json.dumps({"saved": len(saved), "missing": missing, "cache_count": len(list(CACHE.glob("*.json")))}, ensure_ascii=False))
